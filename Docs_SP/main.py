...
def make_pdf(output_path, metadata):
    ...
    story.append(Paragraph('<font color="lightgrey">──────────────────────────────────────────────</font>', styles['BodyTextCustom']))
    ...
    for para in str(metadata.get('explanation', '')).splitlines():
        ...
        if para.startswith(('- ', '• ')):
            para = para[2:].strip()
            story.append(Paragraph(f'• {para}', styles['BulletCustom']))
        else:
...
def process_all_procs(conn, database, output_dir, model="llama3:latest", onedrive=False, onedrive_folder=None):
    ...
    print(f"📦 Found {len(rows)} stored procedures.")
